...
# DEPENDENCIES #
################
pytorch, sklearn, nltk, sklearn-crfsuite, bs4, pandas, numpy, transformers (library for importing BERT)
...
data 
	...
	↳ data4_annotated_for_ref
		...
		↳ train 2
			*_annotated.tsv - smaller subset of training data above
...
bert.py - train BertForTokenClassification model for word-embeddings of input dialog, and feed them to a feed forward nueral network for multi class classification.

	** NOTE - invocation for task 1 (classifying referring expressions) will take multiple hours to train and run to completion **
	invocation: python3 bert.py --num_turns [n] --task [1, 2] --E [num_epochs] -B [batch size] -L [learning_rate]
...
./bert_classify_referents_6_history.sh
./bert_classify_referents_10_history.sh
./bert_classity_refexp.sh     ** NOTE - will take multiple hours to run (not GPU compatible)
